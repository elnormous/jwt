...
func TestNewTokenNone(t *testing.T) {
	testCases := []struct {
		...
		{"Empty token", "{}", "test", "eyJhbGciOiJub25lIiwidHlwIjoiSldUIn0.e30."},
		{"Key value", "{\"a\":\"b\"}", "test", "eyJhbGciOiJub25lIiwidHlwIjoiSldUIn0.eyJhIjoiYiJ9."},
	}

	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			result, _ := jwt.NewToken([]byte(testCase.payload), jwt.AlgNone, []byte(testCase.key))
...
func TestNewTokenHS256(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			result, _ := jwt.NewToken([]byte(testCase.payload), jwt.AlgHS256, []byte(testCase.key))
